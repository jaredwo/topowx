...
import os
from twx.db import add_utc_offset, create_nnr_subset, \
create_thickness_nnr_subset, create_nnr_subset_nolevel
from twx.utils import YEAR, get_days_metadata
import numpy as np
...
if __name__ == '__main__':
    ...
    START_YEAR_STNOBS = 1895
    END_YEAR_STNOBS = 2015
    START_YEAR_RA = 1948
    END_YEAR_RA = 2015  # needs be end year of target period-of-record +1
    ...
    FPATH_NNR_SUBSETS = os.path.join(FPATH_NNR, 'n_america_subset')
    ...
    path_homog_db = os.path.join(FPATH_STNDATA, 'all', 'tair_homog_%s_%s.nc' % (START_YEAR_STNOBS, END_YEAR_STNOBS))
    geonames_usrname = open('/home/jared.oyler/.geonames_username').readline().strip()
    add_utc_offset(path_homog_db, geonames_usrname=geonames_usrname)
    
    # Create NCEP/NCAR Reanalysis North American subsets of variables used in the
    # infilling of missing station observations. 
    days = get_days_metadata(datetime(START_YEAR_RA, 1, 1), datetime(END_YEAR_RA, 12, 31))
