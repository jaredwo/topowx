...
import numpy as np
import datetime
from twx.utils import status_check, get_days_metadata, ymdL, ymdL_to_date, DATE, YMD, YEAR, MONTH, DAY
from netCDF4 import Dataset
from netCDF4 import date2num
import netCDF4
from netcdftime import num2date
import sys
from twx.db import STN_NAME, ELEV, LAT, LON, STATE, station_data_ncdb
...
class insert_china(Insert):
    ...
    def __init__(self, path_data, min_date, max_date, raster_mask=None):
        ...
        self.days = get_days_metadata(min_date, max_date)
...
class InsertGhcn(Insert):
    ...
    def __init__(self, path_stn_file, path_obs_files, min_date, max_date):
        '''
        Parameters
        ----------
        path_stn_file : str
        ...
        for line in afile.readlines():
            ...
            if fips_code in self.VALID_FIPS_CODES:
                ...
                if state not in self.RM_STATES and network_id not in self.RM_NETWORK_CODES:
                    ...
                    if os.path.exists(os.path.join(self.path_obs_files, "%s.dly" % (stn_id_orig,))):
...
class insert_usfs(Insert):
    ...
    def __init__(self, path_obs_files, min_ymd, max_ymd, raster_mask=None):
        ...
        min_date, max_date = ymdL_to_date(min_ymd), ymdL_to_date(max_ymd)
        Insert.__init__(self, min_date, max_date, raster_mask)
        self.path_obs_files = path_obs_files
