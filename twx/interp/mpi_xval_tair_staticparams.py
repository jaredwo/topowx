...
def proc_work(params,rank):
    ...
    krig_params = it.KrigParamsStatic2(stn_slct, None, None, None, None, None)
    krig = it.KrigTair2(stn_slct, krig_params)
    
    gwr_pca = it.GwrPcaTairStatic(stn_slct,params[P_PATH_CLIB],None,None,None,set_pt=False)
    ...
    while 1:
        ...
        if status.tag == TAG_STOPWORK:
            MPI.COMM_WORLD.send([None]*12, dest=RANK_WRITE, tag=TAG_STOPWORK)
            ...
            try:
                ...
                stn_mean_params = p_mean[xval_stn[NEON]]
                krig_params.reset_params(stn_mean_params[0], stn_mean_params[1], stn_mean_params[2], 
                                         stn_mean_params[3], stn_mean_params[4])
                
                stn_anom_params = p_anom[xval_stn[NEON]]
                gwr_pca.reset_params(stn_anom_params[0], stn_anom_params[1], stn_anom_params[2])
                ...
                gwr_pca.setup_for_pt(xval_stn,rm_stnid)
                tair_daily = gwr_pca.gwr_pca()
                ...
                anom_r2 = r_value**2 #r-squared value; variance explained
                ...
                overall_r2 = r_value**2 #r-squared value; variance explained
            ...
            MPI.COMM_WORLD.send((stn_id,mean_mae,mean_bias,anom_mae,anom_bias,anom_r2,overall_mae,overall_bias,overall_r2,std_err,cir,in_ci), dest=RANK_WRITE, tag=TAG_DOWORK)
...
def proc_write(params,nwrkers):
    ...
    while 1:
        ...
        stn_id,mean_mae,mean_bias,anom_mae,anom_bias,anom_r2,overall_mae,overall_bias,overall_r2,std_err,cir,in_ci = MPI.COMM_WORLD.recv(source=MPI.ANY_SOURCE,tag=MPI.ANY_TAG,status=status)
        if status.tag == TAG_STOPWORK:
...
def proc_coord(params,nwrkers):
    ...
    mask_neons = np.in1d(stns[NEON], params[P_NEON_ECORGN],False)
    print np.sum(mask_neons)  
    #Send stn ids to all processes
    MPI.COMM_WORLD.bcast(stns[STN_ID][mask_neons], root=RANK_COORD)
...
if __name__ == '__main__':
    ...
    params[P_PATH_OUT] = '/projects/daymet2/station_data/infill/infill_fnl/xval/xval_tmax_overall.nc'
    ...
    params[P_PATH_PARAMS_MEAN] = '/projects/daymet2/station_data/infill/infill_fnl/param_files/neon_mean_tmax_params.csv'
    params[P_PATH_PARAMS_ANOM] = '/projects/daymet2/station_data/infill/infill_fnl/param_files/neon_anom_tmax_params.csv'
    params[P_VARNAME] = 'tmax'
    params[P_VARNAME_XVAL] = 'tmax'
    params[P_NEON_ECORGN] = np.array([5,6,7,13,15])
