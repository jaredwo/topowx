'''
A MPI driver for performing "leave one out" cross-validation of prcp interpolation in interp_prcp
...
def proc_work(params,rank):
    ...
    if params[P_NGH_RNG].size == 1:
        
        sngl_stn_slct = station_select(stn_da.stns,params[P_NGH_RNG][0],params[P_NGH_RNG][0]+10)
    ...
    while 1:
        ...
        if status.tag == TAG_STOPWORK:
            ...
            try:
                ...
                if xval_mean >= aoutput.ci[0] and xval_mean <= aoutput.ci[1]:
                    in_ci = True
                else:
                    in_ci = False
            ...
            MPI.COMM_WORLD.send((stn_id,min_ngh,hss,perr_ttlamt,perr_freq,perr_intsy,in_ci), dest=RANK_WRITE, tag=TAG_DOWORK)
...
def proc_write(params,nwrkers):
    ...
    while 1:
        ...
        stn_id,min_ngh,hss,perr_ttlamt,perr_freq,perr_intsy,in_ci = MPI.COMM_WORLD.recv(source=MPI.ANY_SOURCE,tag=MPI.ANY_TAG,status=status)
        if status.tag == TAG_STOPWORK:
            ...
            ds.variables['err_amt'][dim1,dim2] = perr_ttlamt
            ds.variables['err_intsy'][dim1,dim2] = perr_intsy
            ds.variables['err_freq'][dim1,dim2] = perr_freq
            ds.variables['in_ci'][dim1,dim2] = in_ci
            ds.sync()
            
            #print "|".join(["WRITER",stn_id,str(min_ngh),"%.4f"%(hss,),"%.2f"%(perr_ttlamt,)])
...
def create_ncdf(params,stn_ids):
    ...
    err_var = ncdf_file.createVariable('hss','f8',('min_nghs','stn_id'))
    err_var.long_name = "hss"
    err_var.standard_name = "hss"
    err_var.missing_value = netCDF4.default_fillvals['f8']
    
    err_var = ncdf_file.createVariable('err_amt','f8',('min_nghs','stn_id'))
    err_var.long_name = "percentage error amount"
    err_var.units = "percentage"
    err_var.standard_name = "percentage_error_amount"
    err_var.missing_value = netCDF4.default_fillvals['f8']
    
    err_var = ncdf_file.createVariable('err_intsy','f8',('min_nghs','stn_id'))
    err_var.long_name = "percentage error intensity"
    err_var.units = "percentage"
    err_var.standard_name = "percentage_error_intensity"
    err_var.missing_value = netCDF4.default_fillvals['f8']
    
    err_var = ncdf_file.createVariable('err_freq','f8',('min_nghs','stn_id'))
    err_var.long_name = "percentage error frequency"
    err_var.units = "percentage"
    err_var.standard_name = "percentage_error_frequency"
    err_var.missing_value = netCDF4.default_fillvals['f8']
    
    in_ci = ncdf_file.createVariable('in_ci','i1',('min_nghs','stn_id'))
    in_ci.long_name = "in prediction interval"
    in_ci.standard_name = "in prediction interval"
...
if __name__ == '__main__':
    ...
    params[P_PATH_OUT] = '/projects/daymet2/interp_output/wxTopo_tests/gwr_ci_test/xval_prcp.nc'
    params[P_PATH_NEON] = '/projects/daymet2/dem/NEON_DOMAINS/neon_mask3.nc'
    params[P_STNS_PER_RGN] = 100
    params[P_NGH_RNG] = np.arange(38,39)
